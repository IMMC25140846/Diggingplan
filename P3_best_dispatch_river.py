...
def cluster_earthwork(earth_result,n_clusters=10,height_threshold=0.5):
    ...
    for i in range(len(earth_result["square_point"])):
        x_coords = [x - X_OFFSET for x in earth_result["square_point"][i][0]]
        y_coords = [y - Y_OFFSET for y in earth_result["square_point"][i][1]]
...
def vis_earthwork(n_clusters=7, show_centers=True,height_threshold=0.5,if_show=True):
    ...
    if if_show:
        ...
        cmap = plt.cm.viridis
        norm = Normalize(vmin=min(earth_result["height"]), vmax=max(earth_result["height"]))
        ...
        norm_colorbar = Normalize(vmin=0, vmax=max(earth_result["height"]))
        cbar1 = plt.colorbar(plt.cm.ScalarMappable(norm=norm_colorbar, cmap=cmap), ax=ax1)
        cbar1.set_label('挖方高度')
        
        ax1.set_title('挖方区域高度热图')
        ax1.set_xlabel('Y 坐标',fontsize=18)
        ax1.set_ylabel('X 坐标',fontsize=18)
        ...
        if show_centers:
            for i, center in enumerate(cluster_centers):
                ax2.scatter(center[1], center[0], s=200, c='red', marker='X', 
        ...
        ax2.set_xlabel('Y 坐标',fontsize=18)
        ax2.set_ylabel('X 坐标',fontsize=18)
...
def run_for_simulation_river(n_clusters, height_threshold, if_show, num_vehicles, split_mode, vehicle_capacity, max_iterations, 
