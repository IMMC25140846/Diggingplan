...
ax.set_zlabel('挖高 (delta_y)')
ax.set_title('真实点云及挖高分布')

plt.show()

plt.figure(figsize=(10, 6))
plt.contourf(x_mesh, y_mesh, delta_y_mesh, levels=100, cmap='viridis')  
plt.colorbar(label='挖高 (delta_y)')  

plt.xlabel('X 坐标 (真实世界)')
plt.ylabel('Y 坐标 (真实世界)')
